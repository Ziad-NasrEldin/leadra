...
scoped_users as (
  ...
  where actor.role in ('admin', 'sub_admin', 'manager')
    and (actor.role in ('admin', 'sub_admin') or p.team_id = actor.team_id)
...
scoped_units as (
  ...
  cross join current_user_profile actor
  where actor.role in ('admin', 'sub_admin', 'manager')
    and (actor.role in ('admin', 'sub_admin') or u.team_id = actor.team_id)
...
scoped_events as (
  ...
  where actor.role in ('admin', 'sub_admin', 'manager')
    ...
    and (actor.role in ('admin', 'sub_admin') or e.team_id = actor.team_id)
...
overview as (
  select jsonb_build_object(
    ...
    'soldUnits', count(*) filter (where u.archived = false and u.status = 'sold'),
    'soldValue', coalesce(sum(u.total_amount) filter (where u.status = 'sold'), 0),
    ...
    'staleUnits', count(*) filter (where u.archived = false and u.status <> 'sold' and u.updated_at < now() - interval '72 hours')
  ) as value
  from scoped_units u
),
sales_performance as (
  ...
  left join lateral (
    select
      ...
      coalesce(sum(total_amount) filter (where status = 'sold'), 0) as sold_value,
      coalesce(sum(commission_amount) filter (where status = 'sold'), 0) as commission_contribution
    from scoped_units u
...
inventory_health as (
  ...
  from (
    select jsonb_build_object(
      ...
      'soldUnits', count(*) filter (where u.status = 'sold'),
      ...
      'staleUnits', count(*) filter (where u.status <> 'sold' and u.updated_at < now() - interval '72 hours')
    ) as item
    from scoped_units u
...
target_progress as (
  ...
  left join lateral (
    select
      ...
      count(distinct u.id) filter (where u.status = 'sold' and u.updated_at between t.starts_at and t.ends_at) as units_sold,
      coalesce(sum(u.total_amount) filter (where u.status = 'sold' and u.updated_at between t.starts_at and t.ends_at), 0) as sold_value,
      coalesce(sum(u.commission_amount) filter (where u.status = 'sold' and u.updated_at between t.starts_at and t.ends_at), 0) as commission,
  ...
  where actor.role in ('admin', 'sub_admin', 'manager')
    and (actor.role in ('admin', 'sub_admin') or (t.scope_type = 'team' and t.scope_id = actor.team_id))
),
filter_options as (
  select jsonb_build_object(
...
select case
  when not exists (select 1 from current_user_profile where role in ('admin', 'sub_admin', 'manager')) then
    ...
    'scopeLabel', case
      when (select role from current_user_profile) = 'manager' then 'Team ' || (select team_id::text from current_user_profile)
      else 'Company-wide'
    end,
