alter table public.units
  add column if not exists garden_area numeric(14, 2),
  add column if not exists terrace_area numeric(14, 2),
  add column if not exists transfer_fees numeric(14, 2),
  add column if not exists maintenance_paid boolean not null default false,
  add column if not exists maintenance_cost numeric(14, 2),
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'units_transfer_fees_non_negative'
      and conrelid = 'public.units'::regclass
  ) then
    alter table public.units
      add constraint units_transfer_fees_non_negative
      check (transfer_fees is null or transfer_fees >= 0);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'units_maintenance_cost_non_negative'
      and conrelid = 'public.units'::regclass
  ) then
    alter table public.units
      add constraint units_maintenance_cost_non_negative
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'units_maintenance_fields_match_toggle'
      and conrelid = 'public.units'::regclass
  ) then
    alter table public.units
      ...
      check (
        ...
        or (not maintenance_paid and maintenance_cost is null and maintenance_due_date is null)
      );
  end if;
end $$;
