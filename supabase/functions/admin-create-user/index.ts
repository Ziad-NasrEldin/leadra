...
serve(async (request) => {
  ...
  if (!['admin', 'sub_admin'].includes(callerProfile.role)) {
    return json({ ok: false, error: 'Only admins can create users.' }, 403)
  }
  ...
  if (!fullName || !email || !password || !jobTitle || !phoneNumber || !role || !status) {
  ...
  if (existingProfile) return json({ ok: false, error: 'A user with this email already exists.' }, 400)
  ...
  if (createError || !createdAuth.user) {
    return json({ ok: false, error: createError?.message ?? 'User creation failed.' }, 400)
  }
  ...
  if (upsertError || !profile) {
    ...
    return json({ ok: false, error: upsertError?.message ?? 'Profile creation failed.' }, 400)
  }
  ...
  await adminClient.from('audit_logs').insert({
    ...
    previous_value: null,
    new_value: { target_user_id: createdAuth.user.id, role },
